...
export function ListOptions({ data, refetchLists, onAddCart,orgId }: ListOptionsProps) {
  ...
  const { mutate: mutateCopy, isLoading: isLoadingCopy } = trpc.list.copyBroadcastList.useMutation({
    onSuccess: ({ list }) => {
  ...
  const { mutate: mutateDelete, isLoading: isLoadingDelete } = trpc.list.deleteBroadcastList.useMutation({
    onSuccess: (list) => {
